...
def launch_gui():
    ...
    # HERO SECTION – Inspired by provided mock‑up
    ...
    input_canvas.create_rounded_rectangle(
        40, 0, 780, 160, radius=25, fill="#ffffff", outline="#e2e8f0", width=1
    )

    input_inner = tk.Frame(input_canvas, bg="#ffffff")

    input_canvas.create_window(410, 80, window=input_inner, width=720, height=140)
    ...
    cfg_header_canvas.create_rounded_rectangle(
        40, 0, 780, 110, radius=25, fill="#f59e0b", outline=""
    )
    ...
    def create_dropdown(frame, label_text, var, choices):
        ...
        card_canvas = tk.Canvas(
            frame, height=100, bg="#f1f5f9", highlightthickness=0, width=350
        )

        card_canvas.pack(side="left", expand=True, padx=10, pady=6)

        card_canvas.create_rounded_rectangle(
            0, 0, 350, 100, radius=20, fill="#ffffff", outline="#e2e8f0", width=1
        )

        inner = tk.Frame(card_canvas, bg="#ffffff")

        card_canvas.create_window(175, 50, window=inner, width=330, height=90)
    ...
    action_canvas.create_window(410, 95, window=action_inner, width=720, height=170)

    # Start button on its own canvas (for custom shadow)

    btn_cnv = tk.Canvas(
        action_inner, width=320, height=68, bg="#ffffff", highlightthickness=0
    )

    btn_cnv.pack(pady=(20, 10))

    # subtle shadow

    btn_cnv.create_rounded_rectangle(
        9, 9, 311, 59, radius=14, fill="#e5e7eb", outline=""
    )

    btn_cnv.create_rounded_rectangle(
        5, 5, 315, 55, radius=14, fill="#16a34a", outline=""
    )
    ...
    btn_cnv.create_window(160, 32, window=start_btn)
